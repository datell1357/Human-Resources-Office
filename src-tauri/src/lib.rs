...
fn parse_accounts(content: &str) -> Vec<ParsedAccount> {
...
fn load_account_file(path: &Path) -> AccountLoadResult {
    let file_name = path
        .file_name()
...
fn resolve_accounts_path(app: &AppHandle) -> PathBuf {
...
#[tauri::command]
fn load_accounts(app: AppHandle) -> AccountLoadResult {
    load_account_file(&resolve_accounts_path(&app))
}

#[cfg_attr(mobile, tauri::mobile_entry_point)]
pub fn run() {
    tauri::Builder::default()
        .invoke_handler(tauri::generate_handler![load_accounts])
