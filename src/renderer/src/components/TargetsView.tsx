...
import { SummaryStrip, type StatusFilter } from './SummaryStrip'
import { TargetTable } from './TargetTable'
import { useAccounts } from '../hooks/useAccounts'

export const TargetsView = (): React.JSX.Element => {
  ...
  const selectFilter = (nextFilter: StatusFilter): void => {
    ...
    const nextRows =
      nextFilter === 'all' ? result?.rows : result?.rows.filter((row) => row.status === nextFilter)
    ...
    <>
      <main className="workspace">
        <header className="workspace-header">
          ...
          {result?.loadedAt && (
            <time dateTime={new Date(result.loadedAt).toISOString()}>
              마지막 판독{' '}
              {new Date(result.loadedAt).toLocaleTimeString('ko-KR', {
                hour: '2-digit',
                minute: '2-digit'
              })}
            </time>
        ...
        <section aria-label="계정 파일 판독 결과" className="content-area">
          {result && (
            <SummaryStrip onSelect={selectFilter} selected={filter} summary={result.summary} />
          )}
